-- ============================================================
-- ICONOFF — REDX Database Schema
-- Portal: redx_ | 27 tables + 1 view
-- Mirrors Sosa Inc architecture with redx_ prefix
-- ============================================================

-- ── FINANCE ────────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS redx_transaction_categories (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  name TEXT NOT NULL, slug TEXT NOT NULL, icon TEXT, color TEXT,
  type TEXT NOT NULL CHECK (type IN ('income', 'expense', 'both')),
  parent_id UUID REFERENCES redx_transaction_categories(id),
  sort_order INT DEFAULT 0, is_default BOOLEAN DEFAULT false, is_active BOOLEAN DEFAULT true,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now()
);
CREATE UNIQUE INDEX idx_redx_cat_user_slug ON redx_transaction_categories(user_id, slug);
ALTER TABLE redx_transaction_categories ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_cat_policy" ON redx_transaction_categories FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_payment_methods (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  name TEXT NOT NULL,
...
ALTER TABLE redx_payment_methods ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_pm_policy" ON redx_payment_methods FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ── CRM (early for FK references) ──────────────────────────────

CREATE TABLE IF NOT EXISTS redx_contacts (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_contacts ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_contacts_policy" ON redx_contacts FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_deals (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  contact_id UUID REFERENCES redx_contacts(id) ON DELETE SET NULL,
...
ALTER TABLE redx_deals ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_deals_policy" ON redx_deals FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ── FINANCE continued ──────────────────────────────────────────

CREATE TABLE IF NOT EXISTS redx_subscriptions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  ...
  category_id UUID REFERENCES redx_transaction_categories(id),
  ...
  payment_method_id UUID REFERENCES redx_payment_methods(id),
  notify_before_renewal BOOLEAN DEFAULT true, notify_days_before INT DEFAULT 3,
  notes TEXT, tags TEXT[],
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(), deleted_at TIMESTAMPTZ
);
CREATE INDEX idx_redx_sub_user_status ON redx_subscriptions(user_id, status);
ALTER TABLE redx_subscriptions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_sub_policy" ON redx_subscriptions FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_invoices (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  invoice_number TEXT NOT NULL, type TEXT NOT NULL CHECK (type IN ('sent', 'received')),
  contact_id UUID REFERENCES redx_contacts(id), contact_name TEXT NOT NULL,
...
CREATE UNIQUE INDEX idx_redx_inv_number ON redx_invoices(user_id, invoice_number);
ALTER TABLE redx_invoices ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_inv_policy" ON redx_invoices FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_transactions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  type TEXT NOT NULL CHECK (type IN ('income', 'expense', 'transfer')),
  amount NUMERIC(12,2) NOT NULL CHECK (amount > 0), currency TEXT NOT NULL DEFAULT 'EUR',
  category_id UUID REFERENCES redx_transaction_categories(id), category_name TEXT NOT NULL, subcategory TEXT,
  ...
  recurring_end_date DATE, parent_recurring_id UUID REFERENCES redx_transactions(id),
  tags TEXT[], receipt_url TEXT, notes TEXT,
  subscription_id UUID REFERENCES redx_subscriptions(id),
  invoice_id UUID REFERENCES redx_invoices(id),
  status TEXT DEFAULT 'completed' CHECK (status IN ('pending', 'completed', 'cancelled', 'refunded')),
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(), deleted_at TIMESTAMPTZ
);
CREATE INDEX idx_redx_tx_user_date ON redx_transactions(user_id, date DESC);
CREATE INDEX idx_redx_tx_user_category ON redx_transactions(user_id, category_name);
CREATE INDEX idx_redx_tx_user_type ON redx_transactions(user_id, type);
ALTER TABLE redx_transactions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_tx_policy" ON redx_transactions FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE TRIGGER redx_transactions_updated BEFORE UPDATE ON redx_transactions
  FOR EACH ROW EXECUTE FUNCTION update_updated_at();

CREATE TABLE IF NOT EXISTS redx_budgets (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  category_id UUID REFERENCES redx_transaction_categories(id), category_name TEXT NOT NULL,
...
CREATE UNIQUE INDEX idx_redx_budget_unique ON redx_budgets(user_id, category_name, month, year);
ALTER TABLE redx_budgets ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_budgets_policy" ON redx_budgets FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE OR REPLACE VIEW redx_budget_summary AS
...
FROM redx_budgets b
LEFT JOIN redx_transactions t ON t.user_id = b.user_id AND t.category_name = b.category_name
...
CREATE TABLE IF NOT EXISTS redx_financial_goals (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_financial_goals ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_goals_policy" ON redx_financial_goals FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_invoice_items (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  invoice_id UUID REFERENCES redx_invoices(id) ON DELETE CASCADE NOT NULL,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_invoice_items ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_inv_items_policy" ON redx_invoice_items FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_finance_snapshots (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
CREATE UNIQUE INDEX idx_redx_snap_unique ON redx_finance_snapshots(user_id, snapshot_date, period_type);
ALTER TABLE redx_finance_snapshots ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_snap_policy" ON redx_finance_snapshots FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Subscription → Transaction function
CREATE OR REPLACE FUNCTION redx_generate_subscription_transaction(sub_id UUID)
RETURNS UUID AS $$
DECLARE
  sub RECORD;
  new_tx_id UUID;
BEGIN
  SELECT * INTO sub FROM redx_subscriptions WHERE id = sub_id AND status = 'active';
  IF sub IS NULL THEN RETURN NULL; END IF;

  INSERT INTO redx_transactions (user_id, type, amount, currency, category_name, category_id, description, date, payment_method, subscription_id, status)
...
CREATE TABLE IF NOT EXISTS redx_notebooks (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_notebooks ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_notebooks_policy" ON redx_notebooks FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_notes (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  notebook_id UUID REFERENCES redx_notebooks(id) ON DELETE SET NULL,
...
ALTER TABLE redx_notes ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_notes_policy" ON redx_notes FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_note_tags (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  note_id UUID REFERENCES redx_notes(id) ON DELETE CASCADE NOT NULL,
  tag TEXT NOT NULL, created_at TIMESTAMPTZ DEFAULT now()
);
CREATE UNIQUE INDEX idx_redx_ntags_unique ON redx_note_tags(note_id, tag);
ALTER TABLE redx_note_tags ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_ntags_policy" ON redx_note_tags FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_note_attachments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  note_id UUID REFERENCES redx_notes(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_note_attachments ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_nattach_policy" ON redx_note_attachments FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ── TASKS & PROJECTS ───────────────────────────────────────────

CREATE TABLE IF NOT EXISTS redx_workspaces (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_workspaces ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_ws_policy" ON redx_workspaces FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_projects (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  workspace_id UUID REFERENCES redx_workspaces(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_projects ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_proj_policy" ON redx_projects FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_tasks (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  project_id UUID REFERENCES redx_projects(id) ON DELETE CASCADE NOT NULL,
  ...
  parent_task_id UUID REFERENCES redx_tasks(id),
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(), deleted_at TIMESTAMPTZ
);
CREATE INDEX idx_redx_tasks_project ON redx_tasks(project_id);
ALTER TABLE redx_tasks ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_tasks_policy" ON redx_tasks FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_task_comments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  task_id UUID REFERENCES redx_tasks(id) ON DELETE CASCADE NOT NULL,
  content TEXT NOT NULL, parent_comment_id UUID REFERENCES redx_task_comments(id),
  is_edited BOOLEAN DEFAULT false,
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(), deleted_at TIMESTAMPTZ
);
ALTER TABLE redx_task_comments ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_tcomm_policy" ON redx_task_comments FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_task_attachments (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  task_id UUID REFERENCES redx_tasks(id) ON DELETE CASCADE NOT NULL,
  file_name TEXT NOT NULL, file_url TEXT NOT NULL, file_type TEXT, file_size_bytes BIGINT, mime_type TEXT,
...
ALTER TABLE redx_task_attachments ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_tattach_policy" ON redx_task_attachments FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_task_time_entries (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  task_id UUID REFERENCES redx_tasks(id) ON DELETE CASCADE NOT NULL,
...
ALTER TABLE redx_task_time_entries ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_time_policy" ON redx_task_time_entries FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ── CRM: Interactions ──────────────────────────────────────────

CREATE TABLE IF NOT EXISTS redx_interactions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  contact_id UUID REFERENCES redx_contacts(id) ON DELETE CASCADE NOT NULL,
  ...
  deal_id UUID REFERENCES redx_deals(id),
  created_at TIMESTAMPTZ DEFAULT now()
);
ALTER TABLE redx_interactions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_interactions_policy" ON redx_interactions FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ── CALENDAR ───────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS redx_events (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  ...
  is_recurring BOOLEAN DEFAULT false, recurrence_rule TEXT, recurrence_end_date DATE, parent_event_id UUID REFERENCES redx_events(id),
  event_type TEXT DEFAULT 'event' CHECK (event_type IN ('event', 'meeting', 'deadline', 'reminder', 'task_due')), color TEXT,
  project_id UUID REFERENCES redx_projects(id), task_id UUID REFERENCES redx_tasks(id),
  contact_id UUID REFERENCES redx_contacts(id), deal_id UUID REFERENCES redx_deals(id),
  reminder_minutes INT[], status TEXT DEFAULT 'scheduled' CHECK (status IN ('scheduled', 'completed', 'cancelled')),
  created_at TIMESTAMPTZ DEFAULT now(), updated_at TIMESTAMPTZ DEFAULT now(), deleted_at TIMESTAMPTZ
);
ALTER TABLE redx_events ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_events_policy" ON redx_events FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE TABLE IF NOT EXISTS redx_reminders (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  title TEXT NOT NULL, notes TEXT, remind_at TIMESTAMPTZ NOT NULL,
  event_id UUID REFERENCES redx_events(id) ON DELETE CASCADE,
  task_id UUID REFERENCES redx_tasks(id) ON DELETE CASCADE,
  subscription_id UUID REFERENCES redx_subscriptions(id) ON DELETE CASCADE,
...
ALTER TABLE redx_reminders ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_reminders_policy" ON redx_reminders FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- ── SETTINGS ───────────────────────────────────────────────────

CREATE TABLE IF NOT EXISTS redx_portal_settings (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
...
CREATE UNIQUE INDEX idx_redx_settings_user ON redx_portal_settings(user_id);
ALTER TABLE redx_portal_settings ENABLE ROW LEVEL SECURITY;
CREATE POLICY "redx_settings_policy" ON redx_portal_settings FOR ALL
  USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
